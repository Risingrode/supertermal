...
const { sqliteExec, sqliteScalar } = require('../lib/sqlite');
...
async function main() {
  ...
  await withServer({
    ...
    assert(sqliteScalar(codexFixture.stateDb, `select count(*) from threads where id='${codexFixture.threadId}'`) === '0', 'Deleting Codex session did not remove thread row');
