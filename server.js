...
function loadModelConfig() {
  ...
  return structuredClone(DEFAULT_MODEL_CONFIG);
}
...
function loadCodexConfig() {
  ...
  return structuredClone(DEFAULT_CODEX_CONFIG);
}
...
function loadDevConfig() {
  ...
  return structuredClone(DEFAULT_DEV_CONFIG);
}
...
function handleSaveDevConfig(ws, msg) {
  if (!msg.config || typeof msg.config !== 'object') {
    return wsSend(ws, { type: 'error', message: '无效的 Host 配置' });
  }
  ...
  wsSend(ws, { type: 'system_message', message: 'Host 配置已保存' });
}
...
function sendSessionList(ws) {
  try {
    wsSend(ws, { type: 'session_list', sessions: buildSessionList() });
  } catch {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'auth') {
      ...
      if (msg.password === PASSWORD || (msg.token && activeTokens.has(msg.token))) {
        ...
        authenticated = true;
        ws._ccAuthenticated = true;  // mark for broadcastSessionList()
...
function handleSaveModelConfig(ws, newConfig) {
  ...
  try {
    for (const file of listSessionJsonFiles()) {
...
function deleteCodexLocalSession(session) {
  ...
  if (!threadId) return { removedFiles: 0, removedDbRows: false, dbError: null };
  ...
  let dbError = null;
  try {
    if (sqliteSupport().available) {
      const quotedThreadId = sqlQuote(threadId);
      sqliteExec(CODEX_STATE_DB_PATH, 'PRAGMA foreign_keys = ON;');
      sqliteExec(CODEX_STATE_DB_PATH, `DELETE FROM thread_dynamic_tools WHERE thread_id = ${quotedThreadId};`);
      sqliteExec(CODEX_STATE_DB_PATH, `DELETE FROM stage1_outputs WHERE thread_id = ${quotedThreadId};`);
      sqliteExec(CODEX_STATE_DB_PATH, `DELETE FROM logs WHERE thread_id = ${quotedThreadId};`);
      sqliteExec(CODEX_STATE_DB_PATH, `DELETE FROM threads WHERE id = ${quotedThreadId};`);
      removedDbRows = true;

      if (fs.existsSync(CODEX_LOG_DB_PATH)) {
        sqliteExec(CODEX_LOG_DB_PATH, `DELETE FROM logs WHERE thread_id = ${quotedThreadId};`);
      }
    }
  } catch (err) {
    dbError = err.message;
  ...
  return { removedFiles, removedDbRows, dbError };
}
...
function handleDetachView(ws, wsId) {
  for (const [, entry] of activeProcesses) {
    if (entry.ws === ws) {
      entry.ws = null;
      entry.wsDisconnectTime = new Date().toISOString();
